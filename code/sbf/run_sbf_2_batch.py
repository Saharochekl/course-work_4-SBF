...
from sbf.sbf_paths import CODE_DIR, PROJECT_ROOT, default_stpsf_data_dir, load_project_json, project_path

from sbf.sbf_campaign_state import CampaignState, canonical_sha256, stable_job_id
from sbf.sbf_campaign_runtime import (
    Deadline,
    SignalController,
    atomic_write_json,
    atomic_write_text,
...
DEFAULT_TEMPLATE = CODE_DIR / "sbf-2.ipynb"
DEFAULT_DATA_ROOT = PROJECT_ROOT / "data"
DEFAULT_RUN_ROOT = PROJECT_ROOT / "runs" / "F150W" / "source"
...
DEFAULT_TARGET_CSV = CODE_DIR / "config" / "targets_go3055_manifest.csv"
...
def validate_notebook_filter_pair(template_path, signal_filter, color_filter):
    ...
    if family == SBF2_NOTEBOOK_FAMILY and pair != CURRENT_NOTEBOOK_FILTER_PAIR:
        ...
        raise ValueError(
            ...
            "Use process.py --filter F090W for the F090W signal campaign."
        )
...
def run_worker(args):
    ...
    batch_root.mkdir(parents=True, exist_ok=True)
    runtime_cache = PROJECT_ROOT / ".cache" / "runtime"
...
def validate_offline_dependencies(args, targets, data_root, batch_root, *, write_report=True):
...
def command_provenance(args):
    ...
    if argv is None:
        ...
        command = [sys.executable, "-m", "sbf.run_sbf_2_batch", *map(str, argv)]
    return {
        "argv": command,
...
def _run_parent_impl(args):
    ...
    with SignalController() as signal_controller:
        for index, (target, job) in enumerate(target_jobs):
            ...
            try:
                proc = launch_process_group(cmd, cwd=CODE_DIR)
            except Exception as exc:
...
def dry_run_parent(args):
    """Read-only input/completion preflight: no campaign, workers or products."""
    validate_parent_args(args)
    data_root = Path(args.data_root)
    targets = load_manifest_targets(args.target_csv, data_root, args.extra_target_csv)
    targets = select_targets(
        targets, args.galaxies, allow_bulk_targets=args.allow_bulk_targets,
        target_keys=args.target_keys, programs=args.programs,
    )
    if not targets:
        raise RuntimeError(f"no targets selected: {args.galaxies}")
    validate_go3055_scope(args, targets, args.template)
    validate_offline_dependencies(
        args, targets, data_root, args.batch_root, write_report=False,
    )
    rows = read_target_status(campaign_root_from_args(args) / TARGET_STATUS_FILENAME)
    reusable = 0
    for target in targets:
        result = None if args.force_reprocess else reusable_result_from_status(rows, target)
        reusable += result is not None
        status = (
            "verified source reused" if result is not None
            else "inputs ready; no verified completion-ledger entry"
        )
        print(f"{target['name']}: {status}")
    print(f"Source completion ledger: {reusable}/{len(targets)} verified")
    print("Dry run complete: no files written, no jobs created, no science executed")
    return 0
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="read-only input and completed-source checks; never start workers",
    )
    ...
    if args.worker and args.dry_run:
        parser.error("--dry-run is a parent-only check; it cannot run a worker")
