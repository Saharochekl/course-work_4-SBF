...
from sbf.sbf_paths import PROJECT_ROOT
...
def load_f150w_reference_center(
    ...
    *,
    ...
    result_path = root / "runs" / "F150W" / "source" / "batch" / f"{slug}_result.json"
    if not result_path.is_file():
        raise FileNotFoundError(f"accepted F150W result is absent: {result_path}")
    result = json.loads(result_path.read_text(encoding="utf-8"))
    from sbf.sbf_paths import project_path
...
from sbf.sbf090_pipeline_support import load_f150w_reference_center, write_dataframe_atomic
