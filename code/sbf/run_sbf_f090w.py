...
_PROJECT_CACHE = PROJECT_ROOT / ".cache" / "runtime"
...
from sbf.run_sbf_2_batch import (
...
from sbf.sbf_campaign_runtime import atomic_write_json, atomic_write_text


DEFAULT_MANIFEST = CODE_DIR / "config" / "targets_go3055_f090w_manifest.csv"
DEFAULT_BASE_NOTEBOOK = CODE_DIR / "sbf-2.ipynb"
DEFAULT_DATA_ROOT = PROJECT_ROOT / "data"
DEFAULT_RUN_ROOT = PROJECT_ROOT / "runs" / "F090W"
...
def materialize_products(
    ...
    for name, source in sources.items():
        readable, error = fits_is_readable(source)
        if not readable:
    ...
    if int(source_result.get("f090_source_schema", 1)) >= F090W_SOURCE_SCHEMA:
        ...
        for name in diagnostic_names:
            destination = Path(diagnostics[name])
    ...
    manifest = {
        ...
        "products": {name: str(path.resolve()) for name, path in sources.items()},
        "diagnostics": {
