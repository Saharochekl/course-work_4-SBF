...
from sbf.sbf_paths import PROJECT_ROOT, load_project_json, project_path
...
def _file_fingerprint(path: Path, hash_small: bool = False) -> dict[str, Any]:
    path = path.resolve()
    stat = path.stat()
    # Cache identity keeps its original spelling when only directories move.
    # These names are never used for I/O or as fallback filesystem locations.
    identity_path = path
    try:
        parts = path.relative_to(PROJECT_ROOT).parts
    except ValueError:
        parts = ()
    if parts[:3] == ("runs", "F150W", "source"):
        identity_path = PROJECT_ROOT / "runs" / "sbf2_go3055"
        identity_path = identity_path.joinpath(*parts[3:])
    elif parts[:2] == ("runs", "F090W"):
        identity_path = PROJECT_ROOT / "runs" / "sbf_f090w_go3055"
        identity_path = identity_path.joinpath(*parts[2:])
    result = {
