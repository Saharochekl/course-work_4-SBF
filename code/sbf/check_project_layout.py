...
From code/: py -m sbf.check_project_layout --with-products
...
def check_sources():
    ...
    scripts = sorted(path for path in code.rglob('*.py') if 'legacy' not in path.parts)
    for path in scripts:
...
def local_paths(value, key=''):
    """RU: пути текущего проекта в JSON. EN: yield in-checkout metadata paths."""
    # Historical provenance is not a required input after the E(k) cache exists.
    if 'fingerprint' in key or key == 'expectation_bootstrapped_from':
        return
    if isinstance(value, dict):
        for name, item in value.items():
            yield from local_paths(item, name)
    elif isinstance(value, list):
        for item in value:
            yield from local_paths(item, key)
...
def check_products():
    ...
    with (PROJECT_ROOT / 'code/config/targets_go3055_manifest.csv').open(newline='') as handle:
        expected = {row['target'] for row in csv.DictReader(handle)}
    groups = {
        'F150 source': sorted((root / 'F150W/source/batch').glob('NGC_*_result.json')),
        'F150 normalized': sorted((root / 'F150W/spectra/batch/results').glob('NGC_*_result.json')),
        'F090 published': sorted((root / 'F090W/products').glob('NGC_*/products.json')),
    }
    issues, refs = [], set()
