...
from sbf.sbf090_pipeline_support import (
...
from sbf.sbf2_normalized_winsor_core import ExperimentConfig
...
class F090WPipelineTests(unittest.TestCase):
    def test_product_manifest_references_files_without_creating_symlinks(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory).resolve()
            paths = campaign_paths(root / "run")
            source_dir = root / "source"
            source_dir.mkdir()
            diagnostic_names = ["center", "isophote_attempts", "isophotes", "external_contaminants"]
            diagnostics = {}
            ...
            source = {
                "stem": "frame", "output_dir": str(source_dir),
                "model_full_fits": str(source_dir / "model.fits"),
                ...
                "f090_isophote_qc": {"passed": True},
            }
            result = {
                "full_normalized_residual_fits": str(source_dir / "full.fits"),
                "normalized_fits": [
                    {"branch": "normalized_full_3p5", "ring": ring,
                     "path": str(source_dir / (ring + ".fits"))}
                    for ring in ["inner", "outer"]
                ],
            }
            with patch("sbf.run_sbf_f090w.source_result_valid", return_value=(True, source, "ok")), \
                 patch("sbf.run_sbf_f090w.fits_is_readable", return_value=(True, "")), \
                 patch("sbf.run_sbf_f090w.build_science_row", return_value={"galaxy": "NGC 4636"}):
                materialize_products(paths, "NGC 4636", result, root)
            product_dir = paths["products"] / "NGC_4636"
            manifest = json.loads((product_dir / "products.json").read_text())
            self.assertEqual(manifest["products"]["normalized_full"], str(source_dir / "full.fits"))
            self.assertEqual(manifest["diagnostics"], diagnostics)
            self.assertTrue(all(not path.is_symlink() for path in product_dir.iterdir()))
            self.assertEqual(len(list(product_dir.iterdir())), 2)
    ...
    def test_every_console_line_has_context_prefix(self):
        ...
        try:
            with patch("sbf.run_sbf_f090w.time.strftime", return_value="13:50:10"):
                timestamped_print(
    ...
    def test_source_schema_rejects_legacy_results_and_gates_new_diagnostics(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            common_patches = (
                patch(
                    "sbf.run_sbf_f090w.input_fingerprint",
                    return_value={"stable": True},
                ),
                patch(
                    "sbf.run_sbf_f090w.fits_is_readable",
                    side_effect=lambda value: (
                        (True, "") if str(value) else (False, "missing path")
                    ),
                ),
                patch(
                    "sbf.run_sbf_f090w.load_f090w_psf_cache",
                    return_value={"psf": np.ones((1, 1))},
                ),
                patch(
                    "sbf.run_sbf_f090w.pd.read_csv",
                    return_value=pd.DataFrame({"row": [1]}),
                ),
                patch(
                    "sbf.run_sbf_f090w._selected_isophote_qc",
                    return_value={
                        "passed": True,
                        "reason": "",
    ...
    def test_targeted_resume_preserves_ten_completed_targets(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("sbf.run_sbf_f090w.final_result_valid", side_effect=final_valid), patch(
                "sbf.run_sbf_f090w.source_result_valid", side_effect=source_valid
            ):
    ...
    def test_new_isophote_schema_schedules_the_same_rebuild_for_all_targets(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = ExperimentConfig()
            with patch(
                "sbf.run_sbf_f090w.final_result_valid",
                return_value=(False, {}, "source stage is not reusable"),
            ), patch(
                "sbf.run_sbf_f090w.source_result_valid",
    ...
    def test_psf_cache_is_atomic_and_reusable(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("sbf.sbf090_pipeline_support.atomic_write_text") as writer:
                checked = load_f090w_psf_cache(
