...
from sbf import run_sbf_2_batch as batch
...
class DedicatedGo3055RunnerTests(unittest.TestCase):
    ...
    def test_dry_run_cannot_enter_parent_or_worker_execution(self):
        args = batch.parse_args(["--dry-run"])
        with patch.object(batch, "parse_args", return_value=args), \
             patch.object(batch, "dry_run_parent", return_value=0) as check, \
             patch.object(batch, "run_parent") as parent, \
             patch.object(batch, "run_worker") as worker:
            self.assertEqual(batch.main(), 0)
            check.assert_called_once_with(args)
            parent.assert_not_called()
            worker.assert_not_called()
        with patch("sys.stderr", new=io.StringIO()), self.assertRaises(SystemExit):
            batch.parse_args(["--worker", "--dry-run"])

    def test_source_preflight_writes_no_campaign_or_products(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "not_created"
            args = batch.parse_args([
                "--dry-run", "--galaxies", "NGC 1380",
                "--batch-root", str(root / "batch"),
                "--campaign-root", str(root / "campaign"),
                "--products-root", str(root / "products"),
            ])
            with patch.object(batch, "validate_offline_dependencies") as deps, \
                 patch("sys.stdout", new=io.StringIO()) as output:
                self.assertEqual(batch.dry_run_parent(args), 0)
            self.assertFalse(root.exists())
            self.assertFalse(deps.call_args.kwargs["write_report"])
            self.assertIn("NGC 1380: inputs ready; no verified completion-ledger entry", output.getvalue())
