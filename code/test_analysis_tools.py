...
class AnalysisToolsTests(unittest.TestCase):
    def test_imports_never_run_analysis_or_write_products(self):
        modules = [
            ...
            "build_sbf_f090w_graph_notebook", "summarize_normalized_winsor_distances",
        ]
    ...
    def test_distance_conversion_and_quadrature(self):
        from build_sbf2_article_tables import common_part, distance_from_modulus, distance_uncertainty
    ...
    def test_generated_notebook_sources_match_saved_notebook(self):
        from build_sbf_f090w_graph_notebook import OUTPUT, build_notebook
    ...
    def test_model_derivatives_on_toy_arrays(self):
        ...
        from build_sbf_f090w_graph_notebook import build_notebook
    ...
    def test_reference_psf_is_unambiguous(self):
        from sbf2_normalized_winsor_recovery import _find_reference_psf
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            products = root / "runs/sbf2_go3055/products/NGC_3379"
            products.mkdir(parents=True)
            with self.assertRaises(FileNotFoundError):
                _find_reference_psf(root)
            first = products / "first_psf_129.fits"
            first.touch()
            self.assertEqual(_find_reference_psf(root), first)
            (products / "ambiguous_psf_129.fits").touch()
            with self.assertRaisesRegex(FileNotFoundError, "найдено 2"):
                _find_reference_psf(root)

    def test_normalized_notebook_uses_explicit_inline_images(self):
        # Static check only: importing recovery selects Agg, so plt.show() is
        # insufficient if these plotting cells are rerun afterwards.
        path = Path(__file__).with_name("sbf-2-normalized-winsor.ipynb")
        notebook = json.loads(path.read_text(encoding="utf-8"))
        for plotter in ("plot_normalized_inputs", "plot_power_comparison"):
            source = next("".join(cell["source"]) for cell in notebook["cells"]
                          if cell["cell_type"] == "code"
                          and f"fig = {plotter}(" in "".join(cell["source"]))
            self.assertIn("display(Image(data=buffer.getvalue()))", source)
            self.assertIn("plt.close(fig)", source)
            self.assertNotIn("plt.show()", source)

    def test_f150w_fit_rejects_invalid_values_and_keeps_annular_aliases(self):
        from scipy.optimize import minimize
        notebook = json.loads(Path(__file__).with_name("sbf-2-graph.ipynb").read_text())
