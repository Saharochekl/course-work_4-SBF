...
class BatchInputContractTests(unittest.TestCase):
    ...
    def test_project_relative_paths_are_independent_of_shell_working_directory(self):
        args = batch.parse_args(
            [
                "--template",
                "code/legacy/review-2026-09-08/sbf3/sbf-3.ipynb",
                "--target-csv",
        ...
        self.assertEqual(Path(args.template), ARCHIVE_DIR / "sbf-3.ipynb")
        self.assertEqual(
    ...
    def test_renamed_sbf3_copy_keeps_layout_guard(self):
        with tempfile.TemporaryDirectory() as tmp:
            renamed = Path(tmp) / "innocent-copy.ipynb"
            renamed.write_bytes((ARCHIVE_DIR / "sbf-3.ipynb").read_bytes())
            self.assertEqual(
...
class Sbf3NotebookContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.notebook = json.loads((ARCHIVE_DIR / "sbf-3.ipynb").read_text())
        cls.source = "\n".join(
            "".join(cell.get("source", []))
