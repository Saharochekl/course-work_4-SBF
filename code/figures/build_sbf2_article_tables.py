#!/usr/bin/env python3
"""Таблицы статьи из готового анализа / Article tables from completed analysis.
...
from sbf.sbf_paths import PROJECT_ROOT, CODE_DIR, load_project_json, project_path
...
ROOT = PROJECT_ROOT
TABLE_DIR = ROOT / "runs" / "F150W" / "analysis" / "tables"
F090_TABLE_DIR = ROOT / "runs" / "F090W" / "analysis" / "tables"
RUN_DIR = ROOT / "runs" / "F150W" / "source"
NORMALIZED_RUN_DIR = ROOT / "runs" / "F150W" / "spectra"
# Accepted result contract, not a new fitting configuration.
ADOPTED_SBF_VERSION = "sbf2-normalized-winsor-v3"
ADOPTED_SBF_BRANCH = "normalized_full_3p5"
# Historical science-freeze identity; do not replace with the current code HEAD.
...
def sha256(path: Path) -> str:
    """Контрольная сумма происхождения / Provenance checksum, streamed in 1-MiB blocks."""
...
def distance_cell(distance: float, uncertainty: float) -> str:
    """Ячейка TeX с явным пропуском / Format a TeX value or missing overlap."""
...
def distance_from_modulus(mu: pd.Series | np.ndarray | float):
    """Модуль в Мпк / Convert modulus to Mpc: mu = 5 log10(D/Mpc) + 25."""
    return 10 ** ((mu - 25) / 5)


def distance_uncertainty(distance, sigma_mu):
    """Линейное распространение sigma / First-order modulus error in Mpc."""
...
def main():
    """Собрать CSV/TeX без перефита / Build CSV/TeX without refitting SBF."""
    TABLE_DIR.mkdir(parents=True, exist_ok=True)
    # Paper III, Table 1 and Eq. (2): slope 1.86 ± 0.16, pivot 1.30,
    # zero point -2.760, population scatter 0.060 mag. These are literature inputs.
    # These are HST/WFC3-IR F110W values,
    ...
    # Central values and both error levels come from the explicit LOO table.
            ...
            long_rows.append(
                {
                    ...
                    "method_label": label,
                    "band": band,
                    "mu_mag": values[mu_col],
                    "sigma_mu_internal_mag": values[mu_internal_col] if mu_internal_col else np.nan,
                    "sigma_mu_common_mag": values[mu_common_col] if mu_common_col else np.nan,
                    "sigma_mu_total_mag": values[mu_total_col],
                    "distance_mpc": values[distance_col],
                    "sigma_distance_internal_mpc": values[distance_internal_col] if distance_internal_col else np.nan,
                    "sigma_distance_common_mpc": values[distance_common_col] if distance_common_col else np.nan,
                    "sigma_distance_total_mpc": values[distance_total_col],
    ...
    software_rows.extend(
        [
            ...
            (
                ...
                sha256(CODE_DIR / "sbf" / "sbf2_normalized_winsor_core.py"),
            ),
            (
                "Normalized SBF runner SHA256",
                sha256(CODE_DIR / "sbf" / "run_sbf_2_normalized_winsor.py"),
            ),
            ...
            (
                "F090W runner SHA256",
                sha256(CODE_DIR / "sbf" / "run_sbf_f090w.py"),
            ),
            (
                "F090W graph builder SHA256",
                sha256(CODE_DIR / "figures" / "build_sbf_f090w_graph_notebook.py"),
            ),
