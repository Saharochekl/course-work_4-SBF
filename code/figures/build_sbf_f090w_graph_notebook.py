...
# Notebook remains at the public code/ entry point, outside helper packages.
OUTPUT = CODE_DIR / "sbf-f090w-graph.ipynb"
...
cells = [
    ...
    PROJECT_ROOT = next(
        (path.resolve() for path in [Path.cwd(), *Path.cwd().parents]
         if (path / "code" / "sbf" / "sbf_paths.py").is_file()),
        None,
    )
    if PROJECT_ROOT is None:
        raise FileNotFoundError("Не найден корень проекта с code/sbf/sbf_paths.py")

    CODE_DIR = PROJECT_ROOT / "code"
    import sys
    if str(CODE_DIR) not in sys.path:
        sys.path.insert(0, str(CODE_DIR))
    from sbf.sbf_paths import PROJECT_ROOT, CODE_DIR, load_project_json, default_stpsf_data_dir
    from figures.publish_article_assets import publish_figure

    DATA_DIR = PROJECT_ROOT / "data"
    RUN_DIR = PROJECT_ROOT / "runs" / "F090W"
    ...
    F150_TABLE = PROJECT_ROOT / "runs" / "F150W" / "analysis" / "tables" / "go3055_calibration_table.csv"
    F150_PRODUCTS_DIR = PROJECT_ROOT / "runs" / "F150W" / "source" / "products"
    DISTANCE_TABLE = CODE_DIR / "reference" / "literature_distance_moduli.csv"
