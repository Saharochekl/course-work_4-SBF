"""Сравнение четырёх порядков винзорирования / Four-branch distance comparison.

Recalibrates saved annular measurements; it never remeasures images. The main
article uses normalized_full_3p5, while the other branches remain controls.
"""
...
TRGB_COMMON_ZEROPOINT_MAG = 0.047  # Published shared Paper IV absolute scale.
N_BOOTSTRAP = 300  # Fixed exploratory Monte Carlo budget, not a physical quantity.
RNG_SEED = 3055  # Reproduce the stored four-branch comparison.
...
def fit_constant(frame):
    """Нуль-пункт + разброс по likelihood / Fit zero point and intrinsic scatter."""
    ...
    # 0.05 initializes scatter; broad positive bounds approximate zero to 1 mag.
    solution = minimize(
...
def bootstrap_zero_point_sigma(training, seed):
    """Разброс нуль-пункта при ресэмплинге / Bootstrap calibration uncertainty."""
